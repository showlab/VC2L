...
def get_pp_render_text(image_size: int=224, font_size: int = 16, max_chars=768,
    ...
    def render_np(text):
        if isinstance(text, list):
            res = [render_np(t) for t in text]
            res = np.stack(res, axis=0)
            return torch.tensor(res)
        ids = np.fromiter(list(text.lower().encode('utf8')), dtype=np.uint16)
        ids = ids[:max_chars]
        ...
        img = to_image_np(letters, image_size)  # A nice square image.
        ...
        if transforms:
            return transforms(Image.fromarray(img).convert('RGB'))
        return Image.fromarray(img).convert('RGB')
    return render_np


if __name__ == '__main__':
    render = get_pp_render_text()
