...
from .model import CLIP, CLIPPO, CustomTextCLIP, convert_weights_to_lp, convert_to_custom_text_state_dict,\
...
def get_tokenizer(model_name, transforms=None):
    from .render_text_np import get_pp_render_text
    return get_pp_render_text(transforms=transforms)

def get_text_tokenizer(model_name):
...
def load_checkpoint(model, checkpoint_path, strict=True):
    ...
    del_keys = [key for key in state_dict.keys() if 'text.' in key]
    for key in del_keys:
        del state_dict[key]
...
def create_model(
    ...
    if pretrained and pretrained.lower() == 'openai':
        ...
        model = CLIPPO(**model_cfg, **model_kwargs, cast_dtype=cast_dtype)
...
def create_model_from_pretrained_with_text(
        model_name: str,
        pretrained: Optional[str] = None,
        precision: str = 'fp32',
        device: Union[str, torch.device] = 'cpu',
